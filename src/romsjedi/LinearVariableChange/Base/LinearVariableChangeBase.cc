...
// -----------------------------------------------------------------------------
  ...
  LinearVariableChangeFactory::LinearVariableChangeFactory
    ...
    if (getMakers().find(name) != getMakers().end()) {
      oops::Log::error() << name
        << " already registered in romsjedi::LinearVariableChangeFactory."
        << std::endl;
      ABORT("Element already registered in romsjedi::"
...
// -----------------------------------------------------------------------------
  ...
  LinearVariableChangeBase * LinearVariableChangeFactory::
                     create(const State & xbg,
                            const State & xfg,
                            const Geometry & geom,
    ...
    typename std::map<std::string, LinearVariableChangeFactory*>::
                                   iterator jloc = getMakers().find(id);
    if (jloc == getMakers().end()) {
      oops::Log::error() << id
        << " does not exist in romsjedi::LinearVariableChangeFactory."
        << std::endl;
      ABORT("Element does not exist in romsjedi::LinearVariableChangeFactory.");
    }
    LinearVariableChangeBase * ptr = jloc->second->make(xbg, xfg, geom, params);
    oops::Log::trace() << "LinearVariableChangeBase::create done"
                       << std::endl;
    return ptr;
  }
...
// -----------------------------------------------------------------------------
  ...
  LinearVariableChangeFactory::createParameters(const std::string &name) {
    typename std::map<std::string, LinearVariableChangeFactory*>::
                                   iterator it = getMakers().find(name);
    if (it == getMakers().end()) {
