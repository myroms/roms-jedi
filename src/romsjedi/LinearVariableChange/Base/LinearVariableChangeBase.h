...
// -----------------------------------------------------------------------------
  ...
  class LinearVariableChangeBase
    : public util::Printable, private boost::noncopyable {
   public:
  ...
  class LinearVariableChangeParametersWrapper : public oops::Parameters {
   ...
   public:
    oops::PolymorphicParameter<romsjedi::LinearVariableChangeParametersBase,
                              ...
                              linearVariableChangeParameters{
                              "linear variable change name", "default", this};
  };
...
// -----------------------------------------------------------------------------
  ...
  class LinearVariableChangeFactory {
   public:
    static LinearVariableChangeBase * create(const State & xbg,
                                             const State & xfg,
                                             const Geometry & geom,
   ...
   private:
    virtual LinearVariableChangeBase * make(const State &,
                                            const State &, const Geometry &,
    ...
    static std::map < std::string,
                      LinearVariableChangeFactory * > & getMakers() {
      static std::map < std::string,
                        LinearVariableChangeFactory * > makers_;
      return makers_;
    }
  };
...
// -----------------------------------------------------------------------------
  ...
  class LinearVariableChangeMaker : public LinearVariableChangeFactory {
    typedef oops::TParameters_IfAvailableElseFallbackType_t
                  <T, GenericLinearVariableChangeParameters> Parameters_;

    LinearVariableChangeBase * make(const State & xbg,
                                    const State & xfg,
                                    const Geometry & geom,
                  const LinearVariableChangeParametersBase & params) override {
      const auto &stronglyTypedParams = dynamic_cast
                                        <const Parameters_&>(params);
      return new T(xbg,
                   xfg,
                   geom,
                   oops::parametersOrConfiguration
                   <oops::HasParameters_<T>::value>(stronglyTypedParams));
    }
