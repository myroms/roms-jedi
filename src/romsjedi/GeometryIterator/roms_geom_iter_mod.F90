...
! ------------------------------------------------------------------------------
!> Get geometry iterator current lat/lon at RHO-points.
...
SUBROUTINE roms_geom_iter_current (self, lon, lat)
  ...
  IF ((self%Iind == -1) .and. (self%Jind == -1)) THEN
    ...
    lon = self%geom%lonr(self%geom%Iend, self%geom%Jend)   ! {-1,-1} means
                                                           ! end of the grid
  ELSE IF ((self%Iind < self%geom%Istr) .or.                                 &
           (self%Iind > self%geom%Iend) .or.                                 &
           (self%Jind < self%geom%Jstr) .or.                                 &
           (self%Jind > self%geom%Jend)) THEN
...
SUBROUTINE roms_geom_iter_next (self)
  ...
  IF (Jind .gt. self%geom%Jend) THEN
    Iind=-1
    Jind=-1
  END IF
