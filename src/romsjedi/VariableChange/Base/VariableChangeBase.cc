...
// -----------------------------------------------------------------------------
  ...
  VariableChangeFactory::VariableChangeFactory(const std::string & name) {
    if (getMakers().find(name) != getMakers().end()) {
      oops::Log::error() << name
        << " already registered in romsjedi::VariableChangeFactory."
        << std::endl;
      ABORT("Element already registered in romsjedi::VariableChangeFactory.");
    }
    getMakers()[name] = this;
  }
...
// -----------------------------------------------------------------------------
  ...
  VariableChangeBase * VariableChangeFactory::
    ...
    if (jloc == getMakers().end()) {
      oops::Log::error() << id
        << " does not exist in romsjedi::VariableChangeFactory."
        << std::endl;
      ABORT("Element does not exist in romsjedi::VariableChangeFactory.");
    }
