...
5. Set environmental variables for the ROMS root directory `ROMS_HOME` (where `roms_src` was
   installed) and its build directory `ROMS_BUILD_DIR` at the command line or in your login script.
   Also, ROMS needs `NF_CONFIG` for the location of the NetCDF Fortran utility for compiling
   and linking.  We also like to define the `MPIRUN` environmental variable to specify the MPI
   executable in a particulat computer (say, `srun`, `mpirun`, etc.).  We need the full path
   for `ctest` to work.
   ```
...
7. Setup the ROMS-JEDI build directory. For simplicity, and unlike other current JEDI projects, the bundle
   ...
   Some computer system require additionak MPI flags.  Use the `MPI_ARGS` macro:
